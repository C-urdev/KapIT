  ...
  return (
    <div className="min-h-screen flex flex-col bg-white dark:bg-gradient-to-b dark:from-[#0a1628] dark:via-[#0f2139] dark:to-[#162842]">
      ...
      <section ref={categoriesRef} className="bg-[#f5f5f2] dark:bg-[#0f2139] scroll-mt-24">
        <div className="w-full max-w-[1700px] mx-auto px-3 sm:px-6 lg:px-8 2xl:px-12 py-14">
          ...
          <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {CATEGORIES.map((cat) => (
              <CategoryCard key={cat.title} icon={cat.icon} title={cat.title} onClick={highlightTopGetStarted} />
            ))}
          </div>
        </div>
      </section>
...
function CategoryCard({ icon: Icon, title, onClick }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="group text-left rounded-2xl bg-white dark:bg-[#162842] border border-[#a3b18a] dark:border-[#1e3a5f] p-6 shadow-sm hover:shadow-lg hover:-translate-y-0.5 transition-all"
    >
