...
export default function UserFaqModal({ isOpen, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-center justify-center p-4 min-[420px]:p-6 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="flex w-full max-w-3xl flex-col bg-[#f8fbf6] dark:bg-[#121416] rounded-2xl shadow-2xl overflow-hidden max-h-[90vh] animate-in zoom-in-95 duration-200">
        ...
        <div className="flex shrink-0 items-center justify-between border-b border-[#e5e7eb] dark:border-[#353c44] p-4 sm:p-5">
          <div className="flex items-center gap-3">
            <div className="flex items-center justify-center w-10 h-10 rounded-full bg-[#f0f4ec] dark:bg-[#353c44]">
              <HelpCircle className="w-5 h-5 text-[#3a5a40] dark:text-[#6f9b74]" />
            </div>
            <div>
        ...
        <main className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 space-y-8 custom-scrollbar">
          ...
          <div className="bg-[#f8fbf6] dark:bg-[#22272b] border border-[#dce5d4] dark:border-[#353c44] rounded-2xl p-5 sm:p-6 shadow-sm">
             <p className="text-[#344e41] dark:text-[#d0d7dd] leading-relaxed text-[15px]">
          ...
          <div className="space-y-8">
            {FAQ_DATA.map((section, idx) => (
              <section key={idx} className="space-y-4">
                <h4 className="text-[17px] font-bold text-[#1c2b1f] border-b border-[#dce5d4] pb-2 dark:text-white dark:border-[#353c44]">{section.category}</h4>
                <div className="space-y-5">
                  {section.items.map((item, itemIdx) => (
                    <div key={itemIdx} className="space-y-1.5">
                      <h5 className="font-semibold text-[#3a5a40] dark:text-[#6f9b74] text-[15px]">{item.question}</h5>
                      <p className="text-[#4b5563] dark:text-[#d0d7dd] leading-relaxed text-[14.5px]">{item.answer}</p>
                    </div>
                  ))}
                </div>
              </section>
          ...
          <div className="bg-[#f0f4ec] dark:bg-[#353c44] rounded-2xl p-5 sm:p-6 mt-8">
             <h4 className="text-[17px] font-bold text-[#1c2b1f] dark:text-white mb-2">Still need help?</h4>
             <p className="text-[#4b5563] dark:text-[#d0d7dd] leading-relaxed text-[14.5px]">
