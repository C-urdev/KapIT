...
const importLegacyPosts = async (req, res) => {
  let client;

  try {
    const payload = Array.isArray(req.body?.posts) ? req.body.posts : [];
    if (!payload.length) {
      return res.json({ success: true, importedCount: 0, skippedCount: 0 });
    }
    ...
    const ownerKey = getUserKey(owner);
    const ownerUsernameKey = String(owner.username || '').trim().toLowerCase();
    const ownerNameKey = String(owner.name || '').trim().toLowerCase();
    const ownerCompanyNameKey = String(owner.company_name || '').trim().toLowerCase();
    let importedCount = 0;
    let skippedCount = 0;

    for (const rawPost of payload) {
      const candidateOwnerKeyFromFields = getUserKey({
        email: rawPost?.ownerEmail || '',
        username: rawPost?.ownerUsername || '',
        name: rawPost?.ownerName || '',
      });
      const candidateOwnerKey = String(
        candidateOwnerKeyFromFields || rawPost?.ownerKey || ''
      )
        .trim()
        .toLowerCase();
      ...
      const isOwnerMatch = [
        ...
        .filter(Boolean)
        .includes(candidateOwnerKey);

      if (!candidateOwnerKey || !isOwnerMatch) {
        skippedCount += 1;
        continue;
      }

      const content = String(rawPost?.content || '').trim();
      const imageUrl = String(rawPost?.imageUrl || '').trim();
      const visibility = normalizeVisibility(rawPost?.visibility);
      const createdAtValue = rawPost?.createdAt ? new Date(rawPost.createdAt) : null;
      const createdAt =
        createdAtValue && Number.isFinite(createdAtValue.getTime())
      ...
      const reactions = parseJsonArray(rawPost?.reactions);
      const comments = parseJsonArray(rawPost?.comments).map(normalizeComment);
      const shares = parseJsonArray(rawPost?.shares);
      const sharedPost = rawPost?.sharedPost && typeof rawPost.sharedPost === 'object' ? rawPost.sharedPost : {};
      ...
      const insertResult = await client.query(
        `INSERT INTO user_posts (
         ...
         SELECT
           $1, $2, $3, $4, $5::jsonb, $6::jsonb, $7::jsonb, $8::jsonb, $9::timestamptz, NOW()
         WHERE NOT EXISTS (
           SELECT 1
           FROM user_posts p
           WHERE p.owner_user_id = $1
             AND COALESCE(p.content, '') = $2
             AND COALESCE(p.image_url, '') = $3
    ...
    console.error('Import legacy posts error:', error);
    return res.status(500).json({
      success: false,
      message: 'Failed to import legacy posts.',
    });
  } finally {
    client?.release();
  }
...
const importLegacySavedPosts = async (req, res) => {
  let client;

  try {
    const payload = Array.isArray(req.body?.savedPosts) ? req.body.savedPosts : [];
    if (!payload.length) {
      return res.json({ success: true, importedCount: 0, skippedCount: 0 });
    }
    ...
    for (const entry of payload) {
      let targetPostId = Number(entry?.id || 0);
      ...
      if (!targetPostId) {
        ...
        const createdAtValue = entry?.createdAt ? new Date(entry.createdAt) : null;
        const createdAt =
          createdAtValue && Number.isFinite(createdAtValue.getTime()) ? createdAtValue.toISOString() : null;
        ...
        const foundBySignature = await client.query(
          `SELECT p.id
           FROM user_posts p
           WHERE p.owner_user_id = $1
             AND COALESCE(p.content, '') = $2
           ...
           ORDER BY p.created_at DESC, p.id DESC
           LIMIT 1`,
          [req.user.id, content, imageUrl, createdAt]
        );

        targetPostId = Number(foundBySignature.rows[0]?.id || 0);
      }
    ...
    console.error('Import legacy saved posts error:', error);
    return res.status(500).json({
      success: false,
      message: 'Failed to import legacy saved posts.',
    });
  } finally {
    client?.release();
  }
