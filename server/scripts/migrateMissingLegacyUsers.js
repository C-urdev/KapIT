...
const main = async () => {
  ...
  try {
    const [legacyUsers, targetUsers] = await Promise.all([
      getUsers(sourcePool),
      getUsers(targetPool),
    ]);

    const targetEmails = new Set(targetUsers.map((row) => String(row.email || '').trim().toLowerCase()));
    const missingUsers = legacyUsers
      .filter((row) => !targetEmails.has(String(row.email || '').trim().toLowerCase()))
      .map(normalizeUser);

    let migratedCount = 0;
    for (const user of missingUsers) {
      await upsertUser(targetPool, user);
      migratedCount += 1;
    }

    console.log(`Migrated ${migratedCount} missing account(s) from legacy database to Supabase.`);
  } finally {
