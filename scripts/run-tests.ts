...
const collectTestFiles = (directory: string): string[] => {
  ...
  for (const entry of entries) {
    ...
    if (entry.isFile() && (entry.name.endsWith('.test.cjs') || entry.name.endsWith('.test.ts'))) {
      files.push(fullPath);
    }
  }
...
const result = spawnSync(process.execPath, ['--require', 'tsx/cjs', '--test', '--test-force-exit', '--test-concurrency=1', ...testFiles], {
  cwd: process.cwd(),
  stdio: 'inherit',
  env: process.env,
});
