// scripts/run-web.ts - Vite launcher
...
try {
  ...
  if (String((error as { code?: string })?.code ?? '') !== 'ERR_MODULE_NOT_FOUND') {
    console.warn('dotenv load skipped:', (error as Error).message);
  }
}
...
try {
  ...
  const canRetry =
    ...
    ((error as { code?: string })?.code === 'EINVAL' || (error as { code?: string })?.code === 'EPERM');
  if (!canRetry) throw error;
  // Retry through cmd.exe on Windows without enabling shell concatenation.
  child = spawn('cmd.exe', ['/d', '/s', '/c', `${command} ${args.join(' ')}`], spawnOptions);
}
