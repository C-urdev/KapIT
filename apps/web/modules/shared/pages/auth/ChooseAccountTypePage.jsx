...
import { registerUser } from '@sharedServices/authService';
      ...
      <main className="flex-1 flex items-center justify-center px-3 sm:px-6 py-8 sm:py-12 bg-gradient-to-br from-[#dad7cd] to-[#f5f5f2] dark:from-[#121416] dark:to-[#22272b]">
        <div className="w-full max-w-3xl">
          <div className="bg-white dark:bg-[#22272b] rounded-2xl border border-[#a3b18a] dark:border-[#353c44] p-5 sm:p-8 shadow-lg dark:shadow-[#6f9b74]/10">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              {ACCOUNT_TYPES.map(({ id, title, subtitle, description, icon: Icon }) => (
                <button
                  ...
                  disabled={loading}
                  className="text-left group relative overflow-hidden bg-white dark:bg-[#1a1d20] border-2 border-[#a3b18a] dark:border-[#444d57] rounded-2xl p-6 hover:border-[#588157] dark:hover:border-[#6f9b74] transition-all duration-300 hover:shadow-xl disabled:opacity-60 disabled:cursor-not-allowed"
                >
                  <div className="flex items-start gap-4">
                    <div className="w-12 h-12 shrink-0 bg-[#f5f5f2] dark:bg-[#353c44] rounded-xl flex items-center justify-center group-hover:scale-105 transition-transform">
                    ...
                    <div>
                      ...
                      <div className="mt-4 text-sm font-semibold text-[#588157] dark:text-[#6f9b74] group-hover:underline">
                        Select →
                      </div>
                    </div>
                  </div>
            ...
            {loading && (
              <p className="mt-5 text-center text-sm text-[#344e41] dark:text-[#d0d7dd]">Creating your account…</p>
            )}
          </div>
        </div>
      </main>
