...
export default function ConfirmModal({
  ...
  const confirmClassName = tone === 'danger'
    ...
    : 'bg-[#3a5a40] hover:bg-[#344e41] dark:bg-[#6f9b74] dark:hover:bg-[#82ad86] text-white';
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/45 px-4 backdrop-blur-sm">
      ...
      />
      <div className="relative w-full max-w-md rounded-3xl border border-[#a3b18a] dark:border-[#353c44] bg-white dark:bg-[#22272b] p-6 shadow-2xl shadow-black/20 transition-colors">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h2 className="text-xl font-extrabold text-[#3a5a40] dark:text-white">{title}</h2>
            <p className="mt-2 text-sm text-[#344e41] dark:text-[#d0d7dd]">{message}</p>
          </div>
          <button
            type="button"
            onClick={onCancel}
        ...
        <div className="mt-6 flex flex-wrap justify-end gap-3">
          <button
            type="button"
            onClick={onCancel}
            className="px-4 py-2.5 rounded-xl border border-[#a3b18a] dark:border-[#444d57] text-[#344e41] dark:text-white hover:bg-[#f5f5f2] dark:hover:bg-[#353c44] transition-colors"
          >
            {cancelLabel}
          </button>
          <button
