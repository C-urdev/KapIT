...
export function ActionButton({ icon: Icon, label, onClick, active = false, accentClass = '', emoji = '', onPointerDown, onPointerUp, onPointerLeave, onContextMenu }) {
  return (
    <button
      ...
      className={`flex items-center justify-center gap-2 rounded-xl px-2 py-3 text-sm font-medium transition-colors ${active ? `bg-[#edf4ff] ${accentClass || 'text-[#2d6cdf]'} dark:bg-[#14304d] dark:text-[#7dc4ff]` : 'text-[#344e41] hover:bg-[#f5f5f2] dark:text-[#d5e6f5] dark:hover:bg-[#1e3a5f]'}`}
    >
