...
export default function CenterFeedShareSheet({ displayName, profileImage, shareMessage, setShareMessage, shareVisibility, setShareVisibility, hasShared, onClose, onShare }) {
  ...
  return (
    <div className="fixed inset-0 z-[95] flex items-end bg-[#344e41]/18 backdrop-blur-[2px] dark:bg-black/35 dark:backdrop-blur-[2px]" onClick={onClose}>
      <div className="max-h-[88vh] w-full overflow-y-auto rounded-t-[2rem] border-t border-[#bfd0af] bg-[linear-gradient(180deg,#ebe7dc_0%,#dad7cd_100%)] px-4 pb-6 pt-3 text-[#344e41] shadow-2xl dark:border-[#2a4a6f] dark:bg-[#23262b] dark:text-white" style={{ paddingBottom: 'max(1.5rem, calc(env(safe-area-inset-bottom) + 1rem))' }} onClick={(event) => event.stopPropagation()}>
        <div className="mx-auto mb-4 h-1.5 w-14 rounded-full bg-[#9caf88] dark:bg-white/30" />
        <div className="mx-auto w-full max-w-4xl rounded-[1.75rem] border border-[#bfd0af] bg-white p-5 shadow-[0_14px_34px_rgba(58,90,64,0.1)] dark:border-[#314a68] dark:bg-[#34343a] dark:shadow-none">
          ...
          <textarea value={shareMessage} onChange={(event) => setShareMessage(event.target.value)} placeholder="Say something about this..." className="mt-4 min-h-28 w-full resize-none rounded-[1.35rem] border border-[#d9dfcf] bg-[#f8fbf6] px-4 py-3 text-[1.02rem] text-[#344e41] outline-none transition-colors placeholder:text-[#6f7d60] focus:border-[#a3b18a] focus:bg-white dark:border-[#41566f] dark:bg-transparent dark:text-white dark:placeholder:text-white/55 dark:focus:border-[#5f87b0] dark:focus:bg-[#3b3d44]" />
        ...
        <section className="mx-auto mt-6 w-full max-w-4xl rounded-[1.75rem] border border-[#bfd0af] bg-white px-5 py-5 shadow-[0_14px_34px_rgba(58,90,64,0.08)] dark:border-[#314a68] dark:bg-[#2b2f35] dark:shadow-none">
