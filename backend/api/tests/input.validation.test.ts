...
test('oversized JSON request returns a 413 response', async () => {
  ...
  const response = await request(app).post('/api/auth/login').set('x-forwarded-for', TEST_IP).send({
...
test('request sanitizer strips null bytes before schema validation', async () => {
  const app = createApp();
  const response = await request(app).post('/api/auth/oauth/state').set('x-forwarded-for', TEST_IP).send({
    provider: 'google\u0000',
    mode: 'signup',
