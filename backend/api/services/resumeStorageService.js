...
const buildStoredFilename = (originalName) => {
  const safeOriginalName = sanitizeOriginalName(originalName)
    ...
    .replace(/^-+|-+$/g, '')
    .slice(0, 80) || 'resume';
  const extension = path.extname(sanitizeOriginalName(originalName)).toLowerCase() || '.pdf';
...
const storeResumeUpload = async ({ buffer, originalName, contentType }) => {
  ...
  const safeOriginalName = sanitizeOriginalName(originalName, extension);
...
const storeGeneratedResumeArtifact = async ({ buffer, fileName }) => {
  ...
  await ensureResumeDirectory();
  const safeFileName = sanitizeOriginalName(fileName, path.extname(String(fileName || '') || '.pdf'));
...
const getResumeDownloadName = (storedName) => {
  ...
  return sanitizeOriginalName(withoutPrefix || `resume${extension}`, extension);
};
