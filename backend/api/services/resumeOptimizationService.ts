...
const normalizeResumeData = (raw, diagnostics = {} as any) => {
  const input = raw && typeof raw === 'object' ? raw as any : {};
  const diagnosticsAny = diagnostics as any;
  ...
  if (!schemaAttempt.success) {
    diagnosticsAny.validation_errors = (schemaAttempt.error?.issues || []).map((issue) => `${issue.path.join('.')}: ${issue.message}`);
  } else {
    diagnosticsAny.validation_errors = [];
  }
  ...
  return {
    ...
    diagnostics: {
      mode: diagnosticsAny.mode || 'model',
      fallback_reason: diagnosticsAny.fallback_reason || '',
      validation_errors: diagnosticsAny.validation_errors || [],
    },
  };
};
...
const parseFallbackStructuredResume = ({ resumeText, preferredRole, fallbackReason }) => {
  const sections = parseSectionsFromText(resumeText) as any;
...
const callGeminiForResume = async ({ resumeText, preferredRole }) => {
  ...
  try {
    ...
    logger.warn({ error: errorAny?.message || String(errorAny) }, 'resume.gemini.fallback');
...
const hasILovePdfCredentials = () => false;

const convertDocxToPdfViaILovePdf = async () => {
  const error = new Error(
    'PDF conversion is not enabled in this build. The backend returns DOCX output and the caller can fall back safely.'
  );
  (error as any).code = 'PDF_CONVERSION_DISABLED';
  throw error;
};
