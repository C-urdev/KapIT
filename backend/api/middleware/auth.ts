...
const readAccessToken = (req: Request): string => readBearerToken(req) || String(req.cookies?.[ACCESS_COOKIE_NAME] || '').trim();

const verifyAccessToken = (token) => normalizeDecodedUser(jwt.verify(token, process.env.JWT_SECRET));

const refreshAccessTokenIfPossible = async (req: Request, res: Response): Promise<any | null> => {
...
const optionalAuth = async (req: Request & { user?: any }, res: Response, next: NextFunction) => {
  try {
    const token = readAccessToken(req);
    if (token) {
      req.user = verifyAccessToken(token);
...
const requireCsrfForCookieAuth = (req: Request, res: Response, next: NextFunction) => {
