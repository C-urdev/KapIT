...
const highRiskAuthLimiterStores = new Set(['auth-api', 'login-attempts', 'auth-attempts', 'forgot-password', 'reset-password']);

const normalizeKey = (value: unknown): string => String(value || '').trim().toLowerCase();
const normalizeRoutePath = (value: unknown): string => String(value || '').trim().toLowerCase().replace(/\/+/g, '/');
...
const getRouteAwareLimiterKeyPrefix = (req: Request): string => {
...
const buildAuthAttemptIdentity = (req: Request & { user?: any }): string => {
...
const getAuthLimiterFailClosedAfterMs = (): number =>
  Math.max(0, Number(process.env.AUTH_LIMITER_FAIL_CLOSED_AFTER_MS || 0));

const getRateLimiterAlertWebhookUrl = (): string =>
  String(process.env.RATE_LIMITER_ALERT_WEBHOOK_URL || '').trim();

const getRateLimiterAlertTimeoutMs = (): number =>
  Math.max(250, Number(process.env.RATE_LIMITER_ALERT_TIMEOUT_MS || 2000));

const getPasswordResetRequestKey = (req: Request): string => {
  const email = normalizeKey(req.body?.email);
  return `${req.ip}:${email || 'anonymous'}`;
};
const getPasswordResetSubmitKey = (req: Request): string => {
...
const getAuthAttemptKey = (req: Request & { user?: any }): string => {
...
const getClientIdentity = (req: Request & { user?: any }): string => normalizeKey(req.user?.id || req.ip || 'anonymous');
const isNonActionableRequest = (req: Request): boolean => ['HEAD', 'OPTIONS'].includes(String(req.method || '').toUpperCase());
const isLocalhostIp = (ip: string | undefined): boolean => {
  const normalized = String(ip || '').trim().toLowerCase();
  return normalized === '127.0.0.1' || normalized === '::1' || normalized === '::ffff:127.0.0.1';
};
const shouldSkipAuthApiRateLimit = (req: Request): boolean => {
...
const setRateLimitHeaders = (res: Response, { max, remaining, resetAt }: any): void => {
...
const shouldFailClosedForStore = (storeName) => {
  ...
  const failClosedAfterMs = getAuthLimiterFailClosedAfterMs();
  if ((!isProduction && !forceFailClosed) || failClosedAfterMs <= 0) {
    return false;
  }
  ...
  return (Date.now() - degradedSince) >= failClosedAfterMs;
};

const emitRateLimiterAlert = (payload) => {
  const webhookUrl = getRateLimiterAlertWebhookUrl();
  if (!webhookUrl || typeof fetch !== 'function') {
    return;
  }

  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), getRateLimiterAlertTimeoutMs());
  timeout.unref?.();
...
const logLimiterDegradedMode = (storeName: string, error?: any) => {
  ...
  const payload = {
    ...
    failClosedAfterMs: getAuthLimiterFailClosedAfterMs(),
...
const clearLoginRateLimit = (req: Request): void => {
  const key = getLoginRateLimitKey(req);
