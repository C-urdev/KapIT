...
const extractSocialLinks = (rawValue: unknown): string[] => {
  ...
  if (typeof rawValue === 'object') {
    const links = SOCIAL_LINK_KEYS.map((key) => toCleanString((rawValue as Record<string, unknown>)?.[key])).filter(Boolean);
...
const normalizeSocialsText = (rawValue: unknown): string => extractSocialLinks(rawValue).join(' | ');
