 ...
 */
const runMigration = async (migrationName: string, up: (client: PoolClient) => Promise<void>): Promise<void> => {
  ...
  try {
    ...
    logger.error({ migrationName, error: error instanceof Error ? error.message : String(error) }, 'Migration failed.');
    throw error;
  } finally {
    client.release();
  }
