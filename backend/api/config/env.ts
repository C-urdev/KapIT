...
const { hasPayPalConfig, getPayPalClientId, getPayPalClientSecret } = require('./paymentEnv.ts');
const { logDemoPricingStartupGuard } = require('./paymentDemoPricing.ts');

let initialized: boolean = false;
let environmentFilesLoaded: boolean = false;

const readEnv = (key: string): string => String(process.env[key] || '').trim();

const isProduction = (): boolean => readEnv('NODE_ENV').toLowerCase() === 'production';

const getEnvironmentFiles = (): Record<string, string> => {
...
const requireValue = (key: string, errors: string[]): void => {
  if (!readEnv(key)) {
    errors.push(`Missing required environment variable: ${key}`);
  }
};

const requireDatabaseConfig = (errors: string[]): void => {
...
const requirePaired = (left: string, right: string, errors: string[]): void => {
...
const validateSecretQuality = (key: string, errors: string[]): void => {
...
const validateUrl = (key: string, errors: string[]): void => {
  const value = readEnv(key);
  if (!value) {
    return;
  }
...
const validateBooleanString = (key: string, errors: string[]): void => {
  const value = readEnv(key);
  if (!value) {
    return;
  }
...
const hasAnyValue = (keys: string[]): boolean => keys.some((key) => Boolean(readEnv(key)));

const requireAtLeastOne = (keys: string[], errors: string[]): void => {
