...
const assertLocalBypassAllowed = (req: Request): void => {
...
const sendRegistrationOtpCode = async (req: Request, res: Response) => {
...
const verifyRegistrationOtpCode = async (req: Request, res: Response) => {
...
const localRegistrationBypass = async (req: Request, res: Response) => {
  try {
    ...
    const message = error instanceof Error ? error.message : 'Localhost registration bypass is unavailable.';
...
const localPasswordResetBypass = async (req: Request, res: Response) => {
  try {
    assertLocalBypassAllowed(req);
    const email = String(req.body?.email || '').trim();
    if (!email) {
    ...
    const message = error instanceof Error ? error.message : 'Localhost password reset bypass is unavailable.';
    return res.status(400).json({
      success: false,
      message,
    });
